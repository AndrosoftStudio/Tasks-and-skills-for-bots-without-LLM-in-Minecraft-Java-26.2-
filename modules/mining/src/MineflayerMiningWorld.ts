...
const blockShape=(b:any):MiningBlock=>({name:b.name,type:b.type,position:{x:b.position.x,y:b.position.y,z:b.position.z},diggable:b.diggable,canHarvest:typeof b.canHarvest==='function'?(id:number|null)=>b.canHarvest(id):undefined,harvestTools:b.harvestTools})
const gravityBlockName=(name:string)=>name==='sand'||name==='red_sand'||name==='gravel'||name==='suspicious_sand'||name==='suspicious_gravel'||name==='anvil'||name==='chipped_anvil'||name==='damaged_anvil'||name==='dragon_egg'||name.endsWith('_concrete_powder')
export class MineflayerMiningWorld implements MiningWorld {
  ...
  canSeeBlock(block:MiningBlock){const b=this.bot.blockAt(v(block.position));if(!b)return false;const canSee=(this.bot as unknown as {canSeeBlock?:(block:unknown)=>boolean}).canSeeBlock;return typeof canSee==='function'?canSee.call(this.bot,b):true}
  isGravityBlock(block:MiningBlock){return gravityBlockName(block.name)}
  ...
  async dig(block:MiningBlock,signal?:AbortSignal){if(signal?.aborted)throw Object.assign(new Error('aborted'),{code:'ABORTED'});const b=this.bot.blockAt(v(block.position));if(!b)throw new Error('block disappeared before dig');const dig=this.bot.dig as unknown as (block:unknown,forceLook?:boolean,digFace?:'auto'|'raycast'|number)=>Promise<void>;await dig.call(this.bot,b,false,'raycast')}

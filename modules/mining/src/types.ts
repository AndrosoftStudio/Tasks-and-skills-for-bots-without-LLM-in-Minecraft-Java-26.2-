...
export interface MiningBlock { name:string; type?:number; position:BlockPos; diggable?:boolean; canHarvest?:(itemType:number|null)=>boolean; harvestTools?:Record<string,unknown> }
...
export interface MiningNavigation {
  ...
  goToPosition?(params:{position:BlockPos;tolerance?:number;profile?:MiningProfile},options?:{signal?:AbortSignal;timeoutMs?:number}):Promise<MiningNavigationResult>
...
export interface MiningWorld {
  ...
  canSeeBlock(block:MiningBlock):boolean
  isGravityBlock(block:MiningBlock):boolean
...
export interface MineBlockParams { position:BlockPos; profile?:MiningProfile; range?:number; requireHarvestable?:boolean; collectDrops?:boolean; collectRadius?:number; maxRepositionAttempts?:number; repositionRadius?:number }
...
export type MiningErrorCode='BLOCK_NOT_FOUND'|'BLOCK_CHANGED'|'BLOCK_NOT_VISIBLE'|'UNREACHABLE'|'NOT_DIGGABLE'|'NO_HARVEST_TOOL'|'DIG_FAILED'|'COLLECT_FAILED'|'ABORTED'|'TIMEOUT'
